...
interface InnovationSubmissionFormProps {
  initialData?: Partial<InnovationFormData & { 
    id?: string;
...
export const InnovationSubmissionForm = ({ 
  initialData, 
  mode = 'create' 
}: InnovationSubmissionFormProps) => {
  ...
  const [pdfNames, setPdfNames] = useState<string[]>(
    initialData?.pdf_urls?.map((_, i) => `Document ${i + 1}`) || []
  );
  ...
  const onSubmit = async (data: InnovationFormData, asDraft: boolean) => {
    ...
    try {
      ...
      // Upload PDFs if new
      const newPdfUrls = await Promise.all(
        pdfFiles.map((file) => uploadFile(file, 'pdfs'))
      );
      const allPdfUrls = [
    ...
    <Form {...form}>
      <form className="space-y-8">
        ...
        <Card className="border-border bg-card">
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2">
              <span className="h-6 w-6 rounded-full bg-primary/20 text-primary flex items-center justify-center text-sm">1</span>
              Basic Details
            </CardTitle>
          </CardHeader>
        ...
        <Card className="border-border bg-card">
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2">
              <span className="h-6 w-6 rounded-full bg-primary/20 text-primary flex items-center justify-center text-sm">2</span>
              Media
            </CardTitle>
          </CardHeader>
        ...
        <Card className="border-border bg-card">
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2">
              <span className="h-6 w-6 rounded-full bg-primary/20 text-primary flex items-center justify-center text-sm">3</span>
              Use Case Story
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Describe the problem scenario, pain points, costs, inefficiencies
                    </FormDescription>
                    <FormControl>
                      <Textarea
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Describe the improved scenario, benefits, measurable outcomes
                    </FormDescription>
                    <FormControl>
                      <Textarea
